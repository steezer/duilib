                              ...
                              BYTE uFade, bool bHole, bool bTiledX, bool bTiledY)
{
    ...
    if (lpAlphaBlend && (bAlpha || uFade < 255))
    {
        ...
        if (rcScale9.top > 0)
        {
            ...
            if (::IntersectRect(&rcTemp, &rcPaint, &rcDest))
            {
                ...
                rcDest.bottom -= rcDest.top;
                lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC,
                             rcBmpPart.left + rcScale9.left, rcBmpPart.top, rcBmpPart.right - rcBmpPart.left - rcScale9.left - rcScale9.right, rcScale9.top, bf);
            }
        }
        // right-top
        ...
        if (rcScale9.left > 0)
        {
            ...
            if (::IntersectRect(&rcTemp, &rcPaint, &rcDest))
            {
                ...
                rcDest.bottom -= rcDest.top;
                lpAlphaBlend(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC,
                             rcBmpPart.left, rcBmpPart.top + rcScale9.top, rcScale9.left, rcBmpPart.bottom - rcBmpPart.top - rcScale9.top - rcScale9.bottom, bf);
            }
        }
        // right
        if (rcScale9.right > 0)
    ...
    {
        if (rc.right - rc.left == rcBmpPart.right - rcBmpPart.left && rc.bottom - rc.top == rcBmpPart.bottom - rcBmpPart.top && rcScale9.left == 0 && rcScale9.right == 0 && rcScale9.top == 0 && rcScale9.bottom == 0)
        {
        ...
        else
        {
            ...
            if (rcScale9.top > 0)
            {
                ...
                if (::IntersectRect(&rcTemp, &rcPaint, &rcDest))
                {
                    ...
                    rcDest.bottom -= rcDest.top;
                    ::StretchBlt(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC,
                                 rcBmpPart.left + rcScale9.left, rcBmpPart.top, rcBmpPart.right - rcBmpPart.left - rcScale9.left - rcScale9.right, rcScale9.top, SRCCOPY);
                }
            }
            // right-top
            if (rcScale9.right > 0 && rcScale9.top > 0)
            ...
            if (rcScale9.left > 0)
            {
                ...
                if (::IntersectRect(&rcTemp, &rcPaint, &rcDest))
                {
                    ...
                    rcDest.bottom -= rcDest.top;
                    ::StretchBlt(hDC, rcDest.left, rcDest.top, rcDest.right, rcDest.bottom, hCloneDC,
                                 rcBmpPart.left, rcBmpPart.top + rcScale9.top, rcScale9.left, rcBmpPart.bottom - rcBmpPart.top - rcScale9.top - rcScale9.bottom, SRCCOPY);
                }
            }
            // right
            if (rcScale9.right > 0)
                              ...
                              TDrawInfo &drawInfo)
{
    ...
    if (drawInfo.pImageInfo == NULL)
    {
        ...
        if (drawInfo.sDrawString.IsEmpty())
            return false;
    ...
    if (drawInfo.rcBmpPart.right > drawInfo.pImageInfo->nX)
        drawInfo.rcBmpPart.right = drawInfo.pImageInfo->nX;
    if (drawInfo.rcBmpPart.bottom > drawInfo.pImageInfo->nY)
        drawInfo.rcBmpPart.bottom = drawInfo.pImageInfo->nY;
    ...
    RECT rcDest = rcItem;
    if (
        drawInfo.rcDestOffset.left != 0 || drawInfo.rcDestOffset.top != 0 ||
    ...
    while (*pstrText != _T('\0'))
    {
        ...
        else if (!bInRaw && (*pstrText == _T('<') || *pstrText == _T('{')) && (pstrText[1] >= _T('a') && pstrText[1] <= _T('z')) && (pstrText[2] == _T(' ') || pstrText[2] == _T('>') || pstrText[2] == _T('}')))
        {
            ...
            case _T('v'): // Vertical Align
            {
                ...
                UINT iVAlign = DT_BOTTOM;
                if (sVAlignStyle.CompareNoCase(_T("center")) == 0)
                    iVAlign = DT_VCENTER;
                else if (sVAlignStyle.CompareNoCase(_T("top")) == 0)
                    iVAlign = DT_TOP;
        ...
        else if (!bInRaw && (*pstrText == _T('<') || *pstrText == _T('{')) && pstrText[1] == _T('/'))
        {
            ...
            case _T('u'):
            {
                ...
                if (pFontInfo == NULL)
                    pFontInfo = pManager->GetFontInfo(iDefaultFont);
        ...
        else if (!bInRaw && *pstrText == _T('<') && pstrText[2] == _T('>') && (pstrText[1] == _T('{') || pstrText[1] == _T('}')))
        {
            ...
            if (bDraw && bLineDraw)
            {
                ...
                if (aVAlignArray.GetSize() > 0)
                    iVAlign = (UINT)aVAlignArray.GetAt(aVAlignArray.GetSize() - 1);
                if (iVAlign == DT_VCENTER)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + (cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading) / 2, &pstrText[1], 1);
                else if (iVAlign == DT_TOP)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y, &pstrText[1], 1);
                else
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, &pstrText[1], 1);
            }
            pt.x += szSpace.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
            cxLine = pt.x - rc.left;
        ...
        else if (!bInRaw && *pstrText == _T(' '))
        {
            ...
            if (bDraw && bLineDraw)
            {
                ...
                if (aVAlignArray.GetSize() > 0)
                    iVAlign = (UINT)aVAlignArray.GetAt(aVAlignArray.GetSize() - 1);
                if (iVAlign == DT_VCENTER)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + (cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading) / 2, _T(" "), 1);
                else if (iVAlign == DT_TOP)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y, _T(" "), 1);
                else
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, _T(" "), 1);
            }
            pt.x += szSpace.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
            cxLine = pt.x - rc.left;
        ...
        else
        {
            ...
            if (bDraw && bLineDraw)
            {
                ...
                if (aVAlignArray.GetSize() > 0)
                    iVAlign = (UINT)aVAlignArray.GetAt(aVAlignArray.GetSize() - 1);
                if (iVAlign == DT_VCENTER)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + (cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading) / 2, pstrText, cchSize);
                else if (iVAlign == DT_TOP)
                    ::TextOut(hDC, pt.x + cxOffset, pt.y, pstrText, cchSize);
                else
                    ::TextOut(hDC, pt.x + cxOffset, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, pstrText, cchSize);

                if (pt.x >= rc.right && (uStyle & DT_END_ELLIPSIS) != 0)
                {
                    if (iVAlign == DT_VCENTER)
                        ::TextOut(hDC, pt.x + cxOffset + szText.cx, pt.y + (cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading) / 2, _T("..."), 3);
                    else if (iVAlign == DT_TOP)
                        ::TextOut(hDC, pt.x + cxOffset + szText.cx, pt.y, _T("..."), 3);
                    else
                        ::TextOut(hDC, pt.x + cxOffset + szText.cx, pt.y + cyLineHeight - pTm->tmHeight - pTm->tmExternalLeading, _T("..."), 3);
                }
            }
            pt.x += szText.cx;
            cxMaxWidth = MAX(cxMaxWidth, pt.x);
...
HBITMAP CRenderEngine::GenerateBitmap(CPaintManagerUI *pManager, RECT rc, CControlUI *pStopControl, DWORD dwFilterColor)
{
    ...
    ASSERT(hPaintDC);
    HBITMAP hPaintBitmap = NULL;
    if (pStopControl == NULL && !pManager->IsLayered())
        hPaintBitmap = pManager->GetPaintOffscreenBitmap();
