...
LPVOID CListUI::GetInterface(LPCTSTR pstrName)
{
    ...
    if (_tcscmp(pstrName, DUI_CTR_ILIST) == 0)
        return static_cast<IListUI *>(this);
    if (_tcscmp(pstrName, DUI_CTR_ILISTOWNER) == 0)
...
{
    if (m_ListInfo.diBk.sDrawString == pStrImage && m_ListInfo.diBk.pImageInfo != NULL)
        return;
...
{
    if (m_ListInfo.diSelected.sDrawString == pStrImage && m_ListInfo.diSelected.pImageInfo != NULL)
        return;
...
{
    if (m_ListInfo.diHot.sDrawString == pStrImage && m_ListInfo.diHot.pImageInfo != NULL)
        return;
...
{
    if (m_ListInfo.diDisabled.sDrawString == pStrImage && m_ListInfo.diDisabled.pImageInfo != NULL)
        return;
...
void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    if (_tcscmp(pstrName, _T("header")) == 0)
        ...
        m_ListInfo.uFixedHeight = _ttoi(pstrValue);
...
{
    if (m_diSep.sDrawString == pStrImage && m_diSep.pImageInfo != NULL)
        return;
    m_diSep.Clear();
...
{
    if (m_cxyFixed.cy == 0)
        return CDuiSize(m_cxyFixed.cx, m_pManager->GetDefaultFontInfo()->tm.tmHeight + 8);
...
{
    if (_tcscmp(pstrName, DUI_CTR_ILISTITEM) == 0)
...
{
    if (pOwner != NULL)
        m_pOwner = static_cast<IListOwnerUI *>(pOwner->GetInterface(DUI_CTR_ILISTOWNER));
}
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT &rcItem)
{
    ...
    if (!IsEnabled())
    {
        if (DrawImage(hDC, pInfo->diDisabled))
            return;
    }
    if (IsSelected())
    {
        if (DrawImage(hDC, pInfo->diSelected))
            return;
    }
    if ((m_uButtonState & UISTATE_HOT) != 0)
    {
        if (DrawImage(hDC, pInfo->diHot))
            return;
    }
...
SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (m_cxyFixed.cx > 0)
    {
        if (m_cxyFixed.cy > 0)
            ...
            return CDuiSize(m_cxyFixed.cx, pInfo->uFixedHeight);
    }
...
SIZE CListTextElementUI::EstimateSize(SIZE szAvailable)
{
    ...
    if (cxyFixed.cx == 0 && pInfo->nColumns > 0)
    {
        ...
        if (m_cxyFixedLast.cx != cxyFixed.cx)
            m_bNeedEstimateSize = true;
    }
    if (cxyFixed.cx > 0)
    {
        if (cxyFixed.cy > 0)
            ...
            return CDuiSize(cxyFixed.cx, pInfo->uFixedHeight);
    }
...
CListContainerElementUI::CListContainerElementUI() : m_iIndex(-1),
                                                     m_iDrawIndex(0),
                                                     m_pOwner(NULL),
                                                     m_bSelected(false),
                                                     m_bExpandable(false),
...
bool CListContainerElementUI::Expand(bool bExpand)
{
    ...
    if (m_bExpandable)
    {
        ...
        {
            if (bExpand)
                m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMEXPAND, false);
            else
                m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMCOLLAPSE, false);
        }
    }
...
{
    if (_tcscmp(pstrName, DUI_CTR_LISTHBOXELEMENT) == 0)
        return static_cast<CListHBoxElementUI *>(this);
...
{
    if (m_pOwner == NULL)
        return CListContainerElementUI::SetPos(rc, bNeedInvalidate);
    ...
    if (pInfo->nColumns > 0)
    {
        ...
        for (int it2 = 0; it2 < m_items.GetSize(); it2++)
        {
            ...
            if (iColumnIndex >= pInfo->nColumns)
                continue;
