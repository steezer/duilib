...
CScrollBarUI::CScrollBarUI() : m_bHorizontal(false),
                               m_nRange(100),
                               m_nScrollPos(0),
...
void CScrollBarUI::SetScrollPos(int nPos, bool bTriggerEvent)
{
    ...
    if (m_nScrollUnit > 1)
    {
        ...
        if (iLeftOffset != 0)
        {
            if (iLeftOffset >= m_nScrollUnit / 2)
                m_nScrollPos += m_nScrollUnit - iLeftOffset;
            else
                m_nScrollPos -= iLeftOffset;
        }
    }
    if (m_nScrollPos > m_nRange)
        m_nScrollPos = m_nRange;
...
{
    if (m_diButton1Normal.sDrawString == pStrImage && m_diButton1Normal.pImageInfo != NULL)
        return;
...
{
    if (m_diButton1Hot.sDrawString == pStrImage && m_diButton1Hot.pImageInfo != NULL)
        return;
...
{
    if (m_diButton1Pushed.sDrawString == pStrImage && m_diButton1Pushed.pImageInfo != NULL)
        return;
...
{
    if (m_diButton1Disabled.sDrawString == pStrImage && m_diButton1Disabled.pImageInfo != NULL)
        return;
...
{
    if (m_diButton2Normal.sDrawString == pStrImage && m_diButton2Normal.pImageInfo != NULL)
        return;
...
{
    if (m_diButton2Hot.sDrawString == pStrImage && m_diButton2Hot.pImageInfo != NULL)
        return;
...
{
    if (m_diButton2Pushed.sDrawString == pStrImage && m_diButton2Pushed.pImageInfo != NULL)
        return;
...
{
    if (m_diButton2Disabled.sDrawString == pStrImage && m_diButton2Disabled.pImageInfo != NULL)
        return;
...
{
    if (m_diThumbNormal.sDrawString == pStrImage && m_diThumbNormal.pImageInfo != NULL)
        return;
...
{
    if (m_diThumbHot.sDrawString == pStrImage && m_diThumbHot.pImageInfo != NULL)
        return;
    m_diThumbHot.Clear();
...
{
    if (m_diThumbPushed.sDrawString == pStrImage && m_diThumbPushed.pImageInfo != NULL)
        return;
...
{
    if (m_diThumbDisabled.sDrawString == pStrImage && m_diThumbDisabled.pImageInfo != NULL)
        return;
...
{
    if (m_diRailNormal.sDrawString == pStrImage && m_diRailNormal.pImageInfo != NULL)
        return;
...
{
    if (m_diRailHot.sDrawString == pStrImage && m_diRailHot.pImageInfo != NULL)
        return;
    m_diRailHot.Clear();
...
{
    if (m_diRailPushed.sDrawString == pStrImage && m_diRailPushed.pImageInfo != NULL)
        return;
...
{
    if (m_diRailDisabled.sDrawString == pStrImage && m_diRailDisabled.pImageInfo != NULL)
        return;
...
{
    if (m_diBkNormal.sDrawString == pStrImage && m_diBkNormal.pImageInfo != NULL)
        return;
    m_diBkNormal.Clear();
...
{
    if (m_diBkHot.sDrawString == pStrImage && m_diBkHot.pImageInfo != NULL)
        return;
    m_diBkHot.Clear();
...
{
    if (m_diBkPushed.sDrawString == pStrImage && m_diBkPushed.pImageInfo != NULL)
        return;
    m_diBkPushed.Clear();
...
{
    if (m_diBkDisabled.sDrawString == pStrImage && m_diBkDisabled.pImageInfo != NULL)
        return;
        ...
        if (::PtInRect(&m_rcButton1, event.ptMouse))
        {
            ...
            if (!m_bHorizontal)
            {
                if (m_pOwner != NULL)
                    ...
                    SetScrollPos(m_nScrollPos - GetLineSize());
            }
            else
            {
                if (m_pOwner != NULL)
...
void CScrollBarUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
    ...
    else if (_tcscmp(pstrName, _T("scrollunit")) == 0)
        SetScrollUnit(_ttoi(pstrValue));
...
void CScrollBarUI::PaintBk(HDC hDC)
{
    ...
    {
        if (DrawImage(hDC, m_diBkDisabled))
            return;
    }
    else if ((m_uThumbState & UISTATE_PUSHED) != 0)
    {
        if (DrawImage(hDC, m_diBkPushed))
            return;
    }
    else if ((m_uThumbState & UISTATE_HOT) != 0)
    {
        if (DrawImage(hDC, m_diBkHot))
            return;
    }

    if (DrawImage(hDC, m_diBkNormal))
        return;
}
...
void CScrollBarUI::PaintButton1(HDC hDC)
{
    ...
    {
        if (m_dwButton1Color >= 0xFF000000)
    ...
    if ((m_uButton1State & UISTATE_DISABLED) != 0)
    {
        ...
        if (DrawImage(hDC, m_diButton1Disabled))
            return;
    }
    else if ((m_uButton1State & UISTATE_PUSHED) != 0)
    {
        m_diButton1Pushed.rcDestOffset = rc;
        if (DrawImage(hDC, m_diButton1Pushed))
            return;
    }
    else if ((m_uButton1State & UISTATE_HOT) != 0)
    {
        m_diButton1Hot.rcDestOffset = rc;
        if (DrawImage(hDC, m_diButton1Hot))
            return;
    }

    m_diButton1Normal.rcDestOffset = rc;
    if (DrawImage(hDC, m_diButton1Normal))
        return;
}
...
void CScrollBarUI::PaintButton2(HDC hDC)
{
    ...
    {
        if (m_dwButton2Color >= 0xFF000000)
    ...
    if ((m_uButton2State & UISTATE_DISABLED) != 0)
    {
        ...
        if (DrawImage(hDC, m_diButton2Disabled))
            return;
    }
    else if ((m_uButton2State & UISTATE_PUSHED) != 0)
    {
        m_diButton2Pushed.rcDestOffset = rc;
        if (DrawImage(hDC, m_diButton2Pushed))
            return;
    }
    else if ((m_uButton2State & UISTATE_HOT) != 0)
    {
        m_diButton2Hot.rcDestOffset = rc;
        if (DrawImage(hDC, m_diButton2Hot))
            return;
    }

    m_diButton2Normal.rcDestOffset = rc;
    if (DrawImage(hDC, m_diButton2Normal))
        return;
}
...
void CScrollBarUI::PaintThumb(HDC hDC)
{
    ...
    if ((m_uThumbState & UISTATE_DISABLED) != 0)
    {
        ...
        if (DrawImage(hDC, m_diThumbDisabled))
            return;
    }
    else if ((m_uThumbState & UISTATE_PUSHED) != 0)
    {
        m_diThumbPushed.rcDestOffset = rc;
        if (DrawImage(hDC, m_diThumbPushed))
            return;
    }
    else if ((m_uThumbState & UISTATE_HOT) != 0)
    {
        m_diThumbHot.rcDestOffset = rc;
        if (DrawImage(hDC, m_diThumbHot))
            return;
    }
    m_diThumbNormal.rcDestOffset = rc;
    if (DrawImage(hDC, m_diThumbNormal))
        return;
}
...
void CScrollBarUI::PaintRail(HDC hDC)
{
    ...
    if ((m_uThumbState & UISTATE_DISABLED) != 0)
    {
        ...
        if (DrawImage(hDC, m_diRailDisabled))
            return;
    }
    else if ((m_uThumbState & UISTATE_PUSHED) != 0)
    {
        m_diRailPushed.rcDestOffset = rc;
        if (DrawImage(hDC, m_diRailPushed))
            return;
    }
    else if ((m_uThumbState & UISTATE_HOT) != 0)
    {
        m_diRailHot.rcDestOffset = rc;
        if (DrawImage(hDC, m_diRailHot))
            return;
    }
    m_diRailNormal.rcDestOffset = rc;
    if (DrawImage(hDC, m_diRailNormal))
        return;
}
